...
int processfile(int numThreads, char * inputfilename , char * outputfilename, double scale)
{
   ...
   int j; //second loop
    ...
    infile = sf_open(inputfilename,SFM_READ, &sf_info);
    ...
    outfile = sf_open(outputfilename,SFM_WRITE, &sf_info);
	...
	numframes = bufferSize/sf_info.channels;
        ...
        for( i = 0; i < numThreads; i++)
        {
             ...
             threadWork[i].scalefactor = scale;    
...
int main(int argc, char ** argv){
   ...
   for(i=0; i < atoi(argv[1]) ; i++){
       processfile(atoi(argv[2]) , argv[3 + i*3] , argv[4 + i*3] , strtod(argv[5 + i*3], NULL));
   }
}
